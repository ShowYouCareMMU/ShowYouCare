...
var get = function get(config, auth, className, objectId, restOptions, clientSDK) {
  var restWhere = { objectId: objectId };
  enforceRoleSecurity('get', className, auth);
  return triggers.maybeRunQueryTrigger(triggers.Types.beforeFind, className, restWhere, restOptions, config, auth, true).then(function (result) {
    restWhere = result.restWhere || restWhere;
    restOptions = result.restOptions || restOptions;
    var query = new RestQuery(config, auth, className, restWhere, restOptions, clientSDK);
    return query.execute();
...
var classesWithMasterOnlyAccess = ['_JobStatus', '_PushStatus', '_Hooks', '_GlobalConfig', '_JobSchedule'];
...
function enforceRoleSecurity(method, className, auth) {
  ...
  if (classesWithMasterOnlyAccess.indexOf(className) >= 0 && !auth.isMaster) {
    var _error = 'Clients aren\'t allowed to perform the ' + method + ' operation on the ' + className + ' collection.';
    throw new Parse.Error(Parse.Error.OPERATION_FORBIDDEN, _error);
  }

  // readOnly masterKey is not allowed
  if (auth.isReadOnly && (method === 'delete' || method === 'create' || method === 'update')) {
    var _error2 = 'read-only masterKey isn\'t allowed to perform the ' + method + ' operation.';
    throw new Parse.Error(Parse.Error.OPERATION_FORBIDDEN, _error2);
  }
}
