...
var transformDotFieldToComponents = function transformDotFieldToComponents(fieldName) {
  return fieldName.split('.').map(function (cmpt, index) {
    if (index === 0) {
      return '"' + cmpt + '"';
    }
    return '\'' + cmpt + '\'';
...
var transformDotField = function transformDotField(fieldName) {
  if (fieldName.indexOf('.') === -1) {
    return '"' + fieldName + '"';
  }
  var components = transformDotFieldToComponents(fieldName);
...
var buildWhereClause = function buildWhereClause(_ref) {
  ...
  var _loop = function _loop(fieldName) {
    ...
    if (fieldName.indexOf('.') >= 0) {
      var name = transformDotField(fieldName);
      if (fieldValue === null) {
        ...
        if (fieldValue.$in) {
          ...
          fieldValue.$in.forEach(function (listElem) {
    ...
    if (Array.isArray(fieldValue.$in) && isArrayField && schema.fields[fieldName].contents && schema.fields[fieldName].contents.type === 'String') {
      ...
      fieldValue.$in.forEach(function (listElem, listIndex) {
        if (listElem === null) {
          ...
          _inPatterns.push('$' + (index + 1 + listIndex - (allowNull ? 1 : 0)));
        }
      });
      if (allowNull) {
        patterns.push('($' + index + ':name IS NULL OR $' + index + ':name && ARRAY[' + _inPatterns.join(',') + '])');
      } else {
        patterns.push('$' + index + ':name && ARRAY[' + _inPatterns.join(',') + ']');
      }
        ...
        if (baseArray.length > 0) {
          ...
          if (isArrayField) {
            ...
            baseArray.forEach(function (listElem, listIndex) {
              if (listElem !== null) {
                values.push(listElem);
                _inPatterns2.push('$' + (index + 1 + listIndex));
              }
            });
            patterns.push('$' + index + ':name ' + not + ' IN (' + _inPatterns2.join(',') + ')');
            index = index + 1 + _inPatterns2.length;
          }
        } else if (!notIn) {
          values.push(fieldName);
          patterns.push('$' + index + ':name IS NULL');
    ...
    if (fieldValue.$geoIntersects && fieldValue.$geoIntersects.$point) {
      var _point = fieldValue.$geoIntersects.$point;
      if ((typeof _point === 'undefined' ? 'undefined' : _typeof(_point)) !== 'object' || _point.__type !== 'GeoPoint') {
        throw new _node2.default.Error(_node2.default.Error.INVALID_JSON, 'bad $geoIntersect value; $point should be GeoPoint');
      } else {
        _node2.default.GeoPoint._validate(_point.latitude, _point.longitude);
      }
      patterns.push('$' + index + ':name::polygon @> $' + (index + 1) + '::point');
      values.push(fieldName, '(' + _point.longitude + ', ' + _point.latitude + ')');
      index += 2;
    }
    ...
    if (fieldValue.$regex) {
      ...
      var _name = transformDotField(fieldName);
      regex = processRegexPattern(regex);

      patterns.push('$' + index + ':raw ' + operator + ' \'$' + (index + 1) + ':raw\'');
      values.push(_name, regex);
      index += 2;
    }
    ...
    if (fieldValue.__type === 'Polygon') {
      var value = convertPolygonToSQL(fieldValue.coordinates);
  ...
  _createClass(PostgresStorageAdapter, [{
    ...
    value: function createTable(className, schema, conn) {
      ...
      return this._ensureSchemaCollectionExists(conn).then(function () {
        ...
        return conn.tx('create-relation-tables', function (t) {
          var queries = relations.map(function (fieldName) {
    ...
    value: function createObject(className, schema, object) {
      ...
      Object.keys(object).forEach(function (fieldName) {
        ...
        switch (schema.fields[fieldName].type) {
          ...
          case 'Polygon':
            {
              var value = convertPolygonToSQL(object[fieldName].coordinates);
      ...
      return this._client.none(qs, values).then(function () {
        ...
        if (error.code === PostgresUniqueIndexViolationError) {
          ...
          if (error.constraint) {
            var matches = error.constraint.match(/unique_([a-zA-Z]+)/);
    ...
    value: function updateObjectsByQuery(className, schema, query, update) {
      ...
      var _loop2 = function _loop2(_fieldName) {
        ...
        if (fieldValue === null) {
          ...
          var _value = convertPolygonToSQL(fieldValue.coordinates);
          updatePatterns.push('$' + index + ':name = $' + (index + 1) + '::polygon');
          values.push(_fieldName, _value);
          index += 2;
        ...
        return results.map(function (object) {
          Object.keys(schema.fields).forEach(function (fieldName) {
            ...
            if (object[fieldName] && schema.fields[fieldName].type === 'Polygon') {
              var coords = object[fieldName];
              coords = coords.substr(2, coords.length - 4).split('),(');
              coords = coords.map(function (point) {
...
function convertPolygonToSQL(polygon) {
  ...
  var unique = polygon.filter(function (item, index, ar) {
    var foundIndex = -1;
